...
void main()
{
    ...
    vec4 clip_space_position = P * V * M * inst_m * vec4(position, 1.0);
    vec3 ndc = clip_space_position.xyz / clip_space_position.w;
    
    // 3. 计算球面坐标（yaw, pitch, radius）
    float radius = length(ndc);  // 半径
    float yaw = -atan(ndc.z, ndc.x);  // 计算yaw（经度），使用反正切函数
    float pitch = asin(ndc.y / radius);  // 计算pitch（纬度），反正弦函数

    // 4. 将 yaw 和 pitch 映射到 [-1, 1] 范围
    float mapped_yaw = yaw / 3.14159265359;  // 映射到 [-1, 1]
    float mapped_pitch = pitch / (3.14159265359 / 2.0);  // 映射到 [-1, 1]

    // 5. 将映射后的 yaw, pitch 和 radius 拼接为一个 vec4
    gl_Position = vec4(mapped_yaw, mapped_pitch, radius, 1.0);
